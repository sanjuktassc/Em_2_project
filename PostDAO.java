...
public class PostDAO {
	...
	public List<Note> getData(int id) {
		...
		try {
			...
			String qu = "select * from notes where userid=? order by pid DESC";
			PreparedStatement ps = conn.prepareStatement(qu);
			ps.setInt(1, id);
	...
	public Note getDataById(int noteId)
	{
		Note p=null;
		try {
			String qu="select * from notes where pid=?";
	...
	public boolean PostUpdate(int nid,String ti,String co)
	{
		...
		try {
			String qu="update notes set title=?,content=? where pid=?";
			PreparedStatement ps=conn.prepareStatement(qu);
			ps.setString(1, ti);
			ps.setString(2,co);
			ps.setInt(3, nid);
	...
	public boolean DeleteNotes(int nid)
	{
		...
		try {
			...
			String qu="delete from notes where pid=?";
			PreparedStatement ps=conn.prepareStatement(qu);
			ps.setInt(1, nid);
